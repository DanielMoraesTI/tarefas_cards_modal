...
function renderSystemInfo() {
    ...
    const envSelect = document.getElementById('envSelect');
    if (appName)
        appName.textContent = info.appName;
    if (appVersion)
...
function renderLogs() {
    ...
    const logsHtml = logs
        ...
        if (log.includes('[Regra Negócio]'))
            logClass += ' log-business';
        if (log.includes('[Stats]'))
            logClass += ' log-stats';
        if (log.includes('[Task]'))
            logClass += ' log-task';
        if (log.includes('[SystemConfig]'))
            logClass += ' log-config';
        if (log.includes('Bloqueio'))
