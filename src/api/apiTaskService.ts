 ...
 */
async function getAllTasks(search: string = '', sort: string = '') {
    try {
 ...
 */
async function updateTask(id: number, task: any) {
    try {
        const res = await fetch(`${BASE_URL}/tasks/${id}`, {
            method: 'PUT',
            headers: {
 ...
 */
async function getTaskTags(taskId: number) {
    try {
 ...
 */
async function addTagToTask(taskId: number, tagId: number) {
    try {
