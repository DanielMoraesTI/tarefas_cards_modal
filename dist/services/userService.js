 ...
 */
export async function loadInitialData(renderCallback) {
    try {
        ...
        if (Array.isArray(users)) {
            users.forEach((userData) => {
                ...
                const newUser = new UserClass(userData.id, userData.name, userData.email, userData.role || UserRole.USER);
 ...
 */
export async function removeUserLogic(id) {
    try {
 ...
 */
export async function createUserBackend(userData) {
    try {
        const newUserData = await apiUserService.createUser(userData);
        const newUser = new UserClass(newUserData.id, newUserData.name, newUserData.email, newUserData.role || UserRole.USER);
        listUsers.push(newUser);
 ...
 */
export async function updateUserBackend(id, updateData) {
    try {
 ...
 */
export async function getUserTasksBackend(userId) {
    try {
 ...
 */
export async function getUserStatsBackend() {
    try {
